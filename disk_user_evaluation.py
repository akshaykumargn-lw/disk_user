...
folder_locations = {
    ...
    2: "/CAE/90_Lauer-Weiss",
    3: "/CAE/02_Daimler_Van",
    4: "/CAE/03_Daimler-EvoBus",
    5: "/CAE/30_Stihl",
    6: "/CAE"
}
...
def get_valid_folder_choice():
    while True:
        ...
        folder_choice = input("Enter the number of your choice: ")
...
df = pd.DataFrame(file_info, columns=["File Name", "Author", "Size", "File Path"])

# Save the Excel file with both selected file type and selected folder in the filename
selected_file_type = selected_file_type.replace("*", "")  # Remove asterisk from the file type
excel_filename = f"Evaluated_Files_{selected_file_type}_{folder_key}.xlsx"
...
while True:
    user_input = input("Enter the size limit (e.g., 100B, 1KB, 10MB): ").strip().upper()
...
df2 = process_size_column(df)

df2 = df2[df2['Size'] >= size_limit]  # Filter based on the numeric 'Size'

def convert_size_column(df2):
    for index, row in df2.iterrows():
        size_value = row['Size']
        if isinstance(size_value, (int, float)):
            if size_value >= 1e9:
                df2.at[index, 'Size'] = f"{size_value / 1e9:.2f}GB"
            elif size_value >= 1e6:
                df2.at[index, 'Size'] = f"{size_value / 1e6:.2f}MB"
            elif size_value >= 1e3:
                df2.at[index, 'Size'] = f"{size_value / 1e3:.2f}KB"
            else:
                df2.at[index, 'Size'] = f"{size_value:.2f}bytes"
    return df2
...
updated_excel_filename = f'Evaluated_Files_{selected_file_type}_{folder_key}_max_size={size_value}{size_unit}.xlsx'
